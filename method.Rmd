...
The word data are not reported because they are irrelevant for our hypotheses.
...
In addition, 24 words were used as motivational filler stimuli, half being of positive and half of negative valence [taken from @klauer_priming_2007]. 
...
Mean visibility was affected by CS presentation duration, `r glm.mean.vis$full$cs_duration`: 
...
Previous work has shown that even clearly subliminal stimuli were able to affect cognitive processes [@van_den_bussche_mechanisms_2009], indicating that, under the present slightly supraliminal presentation conditions, an automatic process should be able to operate on the CSs. 
...
First, we suspected that the instructions' focus on the visibility check (e.g., instructions referred to identification of CSs as the main task; USs were characterized as background pictures) may have interfered with a more holistic default processing mode that is thought to be conducive to EC, or may simply have led to insufficient attention to the USs. 
Second, in the present Experiment 1, participants were asked to evaluate both words and nonwords after the learning phase, using the same rating scale. The presence of words might have restricted the range of nonword evaluations, thereby masking potential EC effects.
...
Third, and most importantly, to investigate whether the absence of EC in Experiment 1 was due to interference from the visibility check, we manipulated the presence versus absence of the visibility check.
...
Finally, Experiment 2 also used a different visibility manipulation: 
...
They were instructed to attend to pictures and words and that we were interested in their overall impression of both. 
...
Evaluative ratings were analyzed in an ANOVA with visibility-check group as between-participants factor, and with masking and US valence as repeated-measures factors. 
...
In this experiment, visibility was high for unmasked CSs and was slightly but not significantly above chance for masked CSs (note here that the power of the visibility test was lower than that of the test for  EC, as only half of the participants entered the former analysis).
A robust EC effect was obtained for unmasked and clearly visible CSs; this finding support the view that inducing a valence focus promotes EC for visible CSs.
In contrast, there was no evidence for EC in the masked condition. 
Hence, under valence-focus instructions EC seems to vanish when reaching the boundaries of conscious perception. 
Perhaps most importantly, this study revealed that the visibility check/identification task did not affect EC: 
the two groups had comparable levels of EC for unmasked CSs. 

#Experiment 3

Experiment 3 realizes another attempt to demonstrate EC for subliminal CSs, and to test whether such an EC effect is modulated by processing goals.
We examined the role of processing goals in EC by directly manipulating this factor. 
That an EC effect was obtained under valence-focus instructions in Experiment 2 but was not obtained under CS-identification instructions in Experiment 1 is consistent with the view that EC depends on processing goals [@corneille_beyond_2009; @gast_what_2011]. 
Experiment 3 moves one step towards incidental learning conditions by comparing the valence-focus instruction to a brightness-judgment orienting task. 
...
This finding is consistent with previous findings suggesting that stimulus valence may be processed unintentionally [@olson_implicit_2009].

# Experiment 4

In Experiment 4, we attempted to further generalize the findings to the visibility manipulation used in Experiment 1, that is, by manipulating presentation duration instead of masking.
...
In contrast to the notion that a response in the orienting task interferes with EC, we found no effect of manipulating the presence versus absence of the response requirement in the orientation task on visibility or EC.
Given this finding, it seems unlikely that orienting responses interfered with visibility or EC effects in the present studies.
...
All participants performed the identification task on all trials; 
...
This finding suggests that CS identification is not sufficient for EC to occur under incidental learning conditions, but that some minimal amount of attentional processing to the USs is necessary.

#Experiment 6

In a final study, we attempted to realize an incidental learning situation similar to the surveillance paradigm [@olson_implicit_2001]. 
...
This is an incidental learning instruction in that it does not focus on valence or on pairings; yet, it ensures that both CS and US stimuli are attended. 
...
procedural features thought to maximize source confusability (and, by implication, an unaware EC effect) are simultaneous presentation onset of CS and US, spatial contiguity or proximity of CS and US, the pairing of a CS with (a sufficient number of) different USs, and high CS (relative to US) salience. 
...
According to @jones_evaluative_2010, a small number of pairings of a CS with different USs may not be sufficient to produce an unaware EC effect. 
...
Using this paradigm, we investigated the notion that the processes underlying such unaware EC effects may also produce EC under subliminal presentation conditions.
...
```{r 'meta-analysis by orientation', fig.width=8, fig.height=8, fig.cap="Meta-analysis of EC effects in the present study (grouped by homogeneous subsets of experimental conditions)"}
...
```{r 'fig2_state_trace_plot', fig.width=6, fig.height=4, fig.cap="EC effects as a function of CS identification (with meta-analytic means and 95% CIs; squares: 30ms; diamonds: 50ms; triangles: 100ms; circles: 900/1000ms; crosses: 30ms non-masked)."}
...
```{r 'fig_vis-by-rating', fig.width=8, fig.height=9, fig.cap="Evaluative ratings of only the briefly presented and masked CSs as a function of CS identification (horizontal axis) and US valence. Symbol represents US valence (plus: positive USs; minus: negative USs; symbols are jittered to avoid overplotting). Lines (solid: positive USs; dashed: negative USs) represent locally smoothed regression models (with 95% confidence bands).", warning=FALSE}
...
To investigate this possibility, we investigated the subset of briefly presented and masked CSs more closely; specifically, we investigated evaluative ratings as a function of US valence and CS identification performance.
...
To evaluate the notion that EC was correlated with visibility even for briefly presented and masked CSs, we tested whether a model that included the US-valence by CS-identification interaction term was better able to account for the data than a model without such a term.
^[We did not analyze Experiment 2 because of the incomplete CS-identification data. For the data from Experiments 1 & 3, model convergence was reached only when some (Exp.1) or all (Exp.3) random item slopes were dropped.]
...
In that group, as depicted in Figure 6, an EC effect tends to emerge for brief and masked CSs when they were correctly identified on 3 or more trials.
...
These results indicate the general lack of a correlation between CS visibility and EC in the set of briefly presented and masked CSs.
